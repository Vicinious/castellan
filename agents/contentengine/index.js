...
function loadConfig(configPath = path.join(__dirname, 'config', 'contentengine.json')) {
...
function createContentengine({ runtime, logger }) {
  ...
  return {
    config,
    runPrimaryLoop(meta = {}) {
      return primaryLoopTask({ db: runtime?.db, logger, taskName: 'primary_loop', summary: meta.summary || 'contentengine primary loop ran' });
    },
    runMaintenance(meta = {}) {
      return maintenanceTask({ logger, scope: meta.scope || 'contentengine' });
    },
    registerHeartbeatTasks() {
      runtime.heartbeat.scheduler.registerTask({
        name: 'contentengine_primary_loop',
      ...
      runtime.heartbeat.scheduler.registerTask({
        name: 'contentengine_maintenance',
