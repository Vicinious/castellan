...
function loadConfig(configPath = path.join(__dirname, 'config', 'inboxagent.json')) {
...
function createInboxagent({ runtime, logger }) {
  ...
  return {
    config,
    runPrimaryLoop(meta = {}) {
      return primaryLoopTask({ db: runtime?.db, logger, taskName: 'primary_loop', summary: meta.summary || 'inboxagent primary loop ran' });
    },
    runMaintenance(meta = {}) {
      return maintenanceTask({ logger, scope: meta.scope || 'inboxagent' });
    },
    registerHeartbeatTasks() {
      runtime.heartbeat.scheduler.registerTask({
        name: 'inboxagent_primary_loop',
