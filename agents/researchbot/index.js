...
function loadConfig(configPath = path.join(__dirname, 'config', 'researchbot.json')) {
...
function createResearchbot({ runtime, logger }) {
  ...
  return {
    config,
    runPrimaryLoop(meta = {}) {
      return primaryLoopTask({ db: runtime?.db, logger, taskName: 'primary_loop', summary: meta.summary || 'researchbot primary loop ran' });
    },
    runMaintenance(meta = {}) {
      return maintenanceTask({ logger, scope: meta.scope || 'researchbot' });
    },
    registerHeartbeatTasks() {
      runtime.heartbeat.scheduler.registerTask({
        name: 'researchbot_primary_loop',
